...
ngal 是一个用 Rust 编写的终端 Galgame（视觉小说）引擎，让你在命令行中体验分支对话的乐趣。它拥有精美的双边框 UI、角色立绘占位、选项分支、存档/读档功能，并会自动创建项目所需的目录和默认剧情文件。
...
· 🛠️ 自动初始化 – 首次运行自动创建 assets/ 和 save/ 文件夹，并生成默认剧情文件
...
```bash
git clone https://github.com/nasyt2331/ngal.git
cd ngal
cargo build --release
```
